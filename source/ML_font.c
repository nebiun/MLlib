...
// Write wchar_t string
u16 ML_DrawTextW(ML_Font *font, int x, int y, const wchar_t *wText)
{	
	bool draw = false;
		...
		if(draw)
		{
			...
			if(glyphData != NULL)
			{
				...
				if(font->style & FONT_STYLE_MASK)
					FreeTypeGX_drawTextFeature(font, x_pos - x_offset - glyphData->glyphAdvanceX, y_pos - y_offset, FreeTypeGX_getWidthEx(font, wText, i), font->style);
				printed++;
			}
		}
...
u16 ML_DrawText(ML_Font *font, int x, int y, const char *text, ...)
{	
	char buffer[1024];
	...
	if(wText) {
		...
		rtn = ML_DrawTextW(font, x, y, wText);
		free(wText);
		return rtn;
	}
	return 0;
...
ML_TextInfo ML_GetTextInfo(ML_Font* font, const char* text, ...)
{
...
bool FreeTypeGX_cacheGlyphData(ML_Font *font, wchar_t charCode)
{
	...
	if (!FT_Load_Glyph(font->ftFace, gIndex, FT_LOAD_DEFAULT | FT_LOAD_RENDER))
	{
		if(font->ftSlot->format == FT_GLYPH_FORMAT_BITMAP)
		{
			...
			if(charData.glyphDataTexture != NULL) 
			{
