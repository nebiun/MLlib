...
{
	_drawImage(&background->image->texObj, background->image->data, background->x, background->y, background->width, background->height, background->scaleX, background->scaleY, background->angle, background->alpha, 0, 0, 0, 0, 0, 0);
}
