...
bool read_png_gx_file_buffer(const u8* file_name, ML_Image *image) 
{
	...
	if(png_sig_cmp((png_bytep)header, 0, 8)){
		ML_Errno = ML_ERRNO(ML_SYSPNG,1);
		return 0;
	}

	if(png_sig_cmp((png_bytep)header, 0, 8)){
		ML_Errno = ML_ERRNO(ML_SYSPNG,2);
		return 0;
	}
	...
	if(!info_ptr){
		png_destroy_read_struct(&png_ptr, (png_infopp)NULL, (png_infopp)NULL);
		ML_Errno = ML_ERRNO(ML_SYSPNG,4);
		return 0;
	}

	if(setjmp(png_jmpbuf(png_ptr))){
		png_destroy_read_struct(&png_ptr, &info_ptr, (png_infopp)NULL);
		ML_Errno = ML_ERRNO(ML_SYSPNG,5);
		return 0;
	}
	...
	image->height = png_get_image_height(png_ptr, info_ptr);
	if(image->width % 4 != 0 || image->height % 4 != 0){
		ML_Errno = ML_ERRNO(ML_SYSPNG,6);
		return 0;
	}
	color_type = png_get_color_type(png_ptr, info_ptr);
	bit_depth = png_get_bit_depth(png_ptr, info_ptr);
	if(bit_depth!=8){
		ML_Errno = ML_ERRNO(ML_SYSPNG,7);
		return 0;
	}
